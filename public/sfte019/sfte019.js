...
$(function(){
	...
	try { startApplication("sfte019"); }catch(ex) { }
	initialApplication();
...
function search(aform) {
	...
	startWaiting();
	jQuery.ajax({
		url: API_URL+"/api/sfte019/search",
...
function save(aform) {
	...
	validSaveForm(function() {
		...
		confirmSave(function() {
			...
			startWaiting();
			jQuery.ajax({
				url: API_URL+"/api/sfte019/insert",
				data: formdata.jsondata,
				headers : formdata.headers,
				type: "POST",
				dataType: "html",
				...
				success: function(data,status,transport){
					...
					successbox(function() { 
						aform.reset();
						setTimeout(function() { $("#nameen").focus(); },200);
					});
...
function update(aform) {
	...
	validSaveForm(function() {
		...
		confirmUpdate(function() {
			...
			startWaiting();
			jQuery.ajax({
				url: API_URL+"/api/sfte019/update",
				data: formdata.jsondata,
				headers : formdata.headers,
				type: "POST",
				dataType: "html",
...
function submitRetrieve(src,site,branch) {
	//#(250000) programmer code begin;
	//#(250000) programmer code end;
	var aform = fslistform;
	aform.site.value = site;
	aform.branch.value = branch;
	let formdata = serializeDataForm(aform);
	startWaiting();
	jQuery.ajax({
		url: API_URL+"/api/sfte019/retrieval",
		data: formdata.jsondata,
		headers: formdata.headers,
		type: "POST",
		dataType: "html",
...
function submitDelete(src,fsParams) {
	...
	confirmDelete([fsParams[1]+"-"+fsParams[2]],function() {
...
function deleteRecord(fsParams) {
	...
	startWaiting();
	jQuery.ajax({
		url: API_URL+"/api/sfte019/remove",
		data: formdata.jsondata,
		headers: formdata.headers,
		type: "POST",
		dataType: "html",
...
function setupDataTable() {
	...
	$("#datatablebody").find(".fa-data-edit").each(function(index,element) {
		$(element).click(function() {
			...
			submitRetrieve(element,$tr.attr("data-site"),$tr.attr("data-key"));
		});
	});
	$("#datatablebody").find(".fa-data-delete").each(function(index,element) {
		$(element).click(function() {
			if($(this).is(":disabled")) return;
			let $tr = $(this).closest("tr");
			submitDelete(element,[$tr.attr("data-site"),$tr.attr("data-key"),$(this).attr("data-name")]);
		});
	});
}
