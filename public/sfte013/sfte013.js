...
$(function(){
	...
	try { startApplication("sfte013"); }catch(ex) { }
	initialApplication();
...
function search(aform) {
	...
	startWaiting();
	jQuery.ajax({
		url: API_URL+"/api/sfte013/search",
...
function insert() {
	...
	aform.template.value = "";
	aform.templatetype.value = "";
	let formdata = serializeDataForm(aform);
	startWaiting();
	jQuery.ajax({
		url: API_URL+"/api/sfte013/add",
		data: formdata.jsondata,
		headers : formdata.headers,
		type: "POST",
		dataType: "html",
...
function save(aform) {
	...
	validSaveForm(function() {
		...
		confirmSave(function() {
			...
			startWaiting();
			jQuery.ajax({
				url: API_URL+"/api/sfte013/insert",
				data: formdata.jsondata,
				headers : formdata.headers,
				type: "POST",
				dataType: "html",
				...
				success: function(data,status,transport){
					...
					successbox(function() { 
						aform.reset();
						setTimeout(function() { $("#template").focus(); },200);
					});
...
function update(aform) {
	...
	validSaveForm(function() {
		...
		confirmUpdate(function() {
			...
			startWaiting();
			jQuery.ajax({
				url: API_URL+"/api/sfte013/update",
				data: formdata.jsondata,
				headers : formdata.headers,
				type: "POST",
				dataType: "html",
...
function submitRetrieve(src,template,templatetype) {
	//#(250000) programmer code begin;
	//#(250000) programmer code end;
	let aform = fslistform;
	aform.template.value = template;
	aform.templatetype.value = templatetype;
	let formdata = serializeDataForm(aform);
	startWaiting();
	jQuery.ajax({
		url: API_URL+"/api/sfte013/retrieval",
		data: formdata.jsondata,
		headers: formdata.headers,
		type: "POST",
		dataType: "html",
...
function submitDelete(src,fsParams) {
	...
	confirmDelete([fsParams[0]+"-"+fsParams[1]],function() {
...
function deleteRecord(fsParams) {
	...
	let params = {
		ajax: true,
		template : fsParams[0],
		templatetype : fsParams[1]
	};
	let formdata = serializeParameters(params);
	startWaiting();
	jQuery.ajax({
		url: API_URL+"/api/sfte013/remove",
		data: formdata.jsondata,
		headers: formdata.headers,
		type: "POST",
		dataType: "html",
...
function setupDialogComponents() {
	...
	try { $("#contents").tinymce().remove(); } catch(ex) { }
	setupEntryComponent();
...
function setupDataTable() {
	...
	$("#datatablebody").find(".fa-data-edit").each(function(index,element) {
		$(element).click(function() {
			...
			submitRetrieve(element,$(this).attr("data-key"),$(this).attr("data-type"));
		});
	});
	$("#datatablebody").find(".fa-data-delete").each(function(index,element) {
		$(element).click(function() {
			if($(this).is(":disabled")) return;
			submitDelete(element,[$(this).attr("data-key"),$(this).attr("data-type")]);
		});
	});
}
