...
function save(aform) {
	...
	validSaveForm(function() {
		...
		confirmSave(function() {
			...
			startWaiting();
			jQuery.ajax({
				url: API_URL+"/api/sfte003/insert",
				data: formdata.jsondata,
				headers : formdata.headers,
				type: "POST",
				dataType: "html",
				...
				success: function(data,status,transport){
					...
					successbox(function() { 
						aform.reset();
						setTimeout(function() { $("#product").focus(); },200);
					});
...
function update(aform) {
	...
	validSaveForm(function() {
		...
		confirmUpdate(function() {
			...
			startWaiting();
			jQuery.ajax({
				url: API_URL+"/api/sfte003/update",
				data: formdata.jsondata,
				headers : formdata.headers,
				type: "POST",
				dataType: "html",
...
function submitRetrieve(src,product) {
	//#(250000) programmer code begin;
	//#(250000) programmer code end;
	let aform = fslistform;
	aform.product.value = product;
	let formdata = serializeDataForm(aform);
	startWaiting();
	jQuery.ajax({
		url: API_URL+"/api/sfte003/retrieval",
		data: formdata.jsondata,
		headers: formdata.headers,
		type: "POST",
		dataType: "html",
