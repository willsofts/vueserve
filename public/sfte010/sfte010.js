...
$(function(){
	...
	try { startApplication("sfte010"); }catch(ex) { }
	initialApplication();
...
function setupComponents() {
	...
	$("#policysettinglinker").click(function() { policySettingClick(); });
	$("#reservewordlinker").click(function() { reserveWordClick(); });
	$("#reservenumberlinker").click(function() { reserveNumberClick(); });
...
function search(aform) {
	...
	startWaiting();
	jQuery.ajax({
		url: API_URL+"/api/sfte010/search",
...
function insert() {
	...
	aform.reservepwd.value = "";
	let formdata = serializeDataForm(aform);
	startWaiting();
	jQuery.ajax({
		url: API_URL+"/api/sfte010/add",
		data: formdata.jsondata,
		headers : formdata.headers,
		type: "POST",
		dataType: "html",
...
function save(aform) {
	...
	fs_requiredfields = { "reservepwd":{msg:""} };
	...
	validSaveForm(function() {
		...
		confirmSave(function() {
			...
			startWaiting();
			jQuery.ajax({
				url: API_URL+"/api/sfte010/insert",
				data: formdata.jsondata,
				headers : formdata.headers,
				type: "POST",
				dataType: "html",
				...
				success: function(data,status,transport){
					...
					successbox(function() { 
						aform.reset();
						setTimeout(function() { $("#reservepwd").focus(); },200);
					});
...
function update(aform) {
	...
	validSaveForm(function() {
		...
		confirmUpdate(function() {
			...
			startWaiting();
			jQuery.ajax({
				url: API_URL+"/api/sfte010/update",
				data: formdata.jsondata,
				headers : formdata.headers,
				type: "POST",
				dataType: "html",
...
function submitRetrieve(src,reservepwd) {
	//#(250000) programmer code begin;
	//#(250000) programmer code end;
	let aform = fslistform;
	aform.reservepwd.value = reservepwd;
	let formdata = serializeDataForm(aform);
	startWaiting();
	jQuery.ajax({
		url: API_URL+"/api/sfte010/retrieval",
		data: formdata.jsondata,
		headers: formdata.headers,
		type: "POST",
		dataType: "html",
...
function deleteRecord(fsParams) {
	...
	let params = {
		ajax: true,
		reservepwd : fsParams[0]
	};
	let formdata = serializeParameters(params);
	startWaiting();
	jQuery.ajax({
		url: API_URL+"/api/sfte010/remove",
		data: formdata.jsondata,
		headers: formdata.headers,
		type: "POST",
		dataType: "html",
