...
$(function(){
	...
	try { startApplication("sfte014"); }catch(ex) { }
	initialApplication();
...
function setupComponents() {
	...
	$("#jsondialogpanel").find(".modal-dialog").draggable();
	$("#exceldialogpanel").find(".modal-dialog").draggable();
	$("#exportjsonbutton").click(function() {
		return exportJsonHandler();
	});
	$("#exportexcelbutton").click(function() { 
		return exportExcelHandler();
	});
	$("#jsonstartuploadbutton").click(function() {
		return uploadDialog('json');
	});
	$("#excelstartuploadbutton").click(function() {
		return uploadDialog('excel');
	});
...
function search(aform) {
	...
	startWaiting();
	jQuery.ajax({
		url: API_URL+"/api/sfte014/search",
...
function insert() {
	...
	aform.labelid.value = "";
	aform.langcode.value = "";
	aform.labelname.value = "";
	let formdata = serializeDataForm(aform);
	startWaiting();
	jQuery.ajax({
		url: API_URL+"/api/sfte014/add",
		data: formdata.jsondata,
		headers : formdata.headers,
		type: "POST",
		dataType: "html",
...
function save(aform) {
	...
	validSaveForm(function() {
		...
		confirmSave(function() {
			...
			startWaiting();
			jQuery.ajax({
				url: API_URL+"/api/sfte014/insert",
				data: formdata.jsondata,
				headers : formdata.headers,
				type: "POST",
				dataType: "html",
				...
				success: function(data,status,transport){
					...
					successbox(function() { 
						aform.reset();
						setTimeout(function() { $("#labelid").focus(); },200);
					});
...
function update(aform) {
	...
	validSaveForm(function() {
		...
		confirmUpdate(function() {
			...
			startWaiting();
			jQuery.ajax({
				url: API_URL+"/api/sfte014/update",
				data: formdata.jsondata,
				headers : formdata.headers,
				type: "POST",
				dataType: "html",
...
function submitRetrieve(src,labelid,langcode,labelname) {
	//#(250000) programmer code begin;
	//#(250000) programmer code end;
	let aform = fslistform;
	aform.labelid.value = labelid;
	aform.langcode.value = langcode;
	aform.labelname.value = labelname;
	let formdata = serializeDataForm(aform);
	startWaiting();
	jQuery.ajax({
		url: API_URL+"/api/sfte014/retrieval",
		data: formdata.jsondata,
		headers: formdata.headers,
		type: "POST",
		dataType: "html",
...
function submitDelete(src,fsParams) {
	...
	confirmDelete([fsParams[2]],function() {
...
function deleteRecord(fsParams) {
	...
	let params = {
		...
		labelid : fsParams[0],
		langcode : fsParams[1],
		labelname : fsParams[2]
	};
	let formdata = serializeParameters(params);
	startWaiting();
	jQuery.ajax({
		url: API_URL+"/api/sfte014/remove",
		data: formdata.jsondata,
		headers: formdata.headers,
		type: "POST",
		dataType: "html",
...
function setupDataTable() {
	...
	$("#datatablebody").find(".fa-data-edit").each(function(index,element) {
		$(element).click(function() {
			...
			submitRetrieve(element,$tr.attr("data-labelid"),$tr.attr("data-langcode"),$tr.attr("data-labelname"));
		});
	});
	$("#datatablebody").find(".fa-data-delete").each(function(index,element) {
		$(element).click(function() {
			if($(this).is(":disabled")) return;
			let $tr = $(this).closest("tr");
			submitDelete(element,[$tr.attr("data-labelid"),$tr.attr("data-langcode"),$tr.attr("data-labelname")]);
		});
	});
}
//#(390000) programmer code begin;
function exportJsonHandler() {
	let fs_params = {type: "json", labelid: $("#labelids").val() };
	openNewWindow({ url: BASE_URL+"/report/sfte014", windowName: "sfte014_json_window", params:  fs_params });
	return false;
}
function exportExcelHandler() {
	let fs_params = {type: "excel", labelid: $("#labelids").val() };
	openNewWindow({ url: BASE_URL+"/report/sfte014", windowName: "sfte014_excel_window", params:  fs_params });
	return false;
}
...
function upload(filetype) {
	...
	jQuery.ajax({
		...
		success: function(data,status,transport){ 
			...
			stopWaiting();
			successbox(() => {
				$("#fsmodaldialog_layer"+filetype).modal("hide");
			});
			search();
		}
	});	
