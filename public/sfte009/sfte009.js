...
$(function(){
	...
	try { startApplication("sfte009"); }catch(ex) { }
	initialApplication();
...
function search(aform) {
	...
	startWaiting();
	jQuery.ajax({
		url: API_URL+"/api/sfte009/search",
...
function insert() {
	...
	aform.domainid.value = "";
	let formdata = serializeDataForm(aform);
	startWaiting();
	jQuery.ajax({
		url: API_URL+"/api/sfte009/add",
		data: formdata.jsondata,
		headers : formdata.headers,
		type: "POST",
		dataType: "html",
...
function save(aform) {
	...
	fs_requiredfields["basedn"] = {msg:""};
	if("S"==$("#domaintype").val()) {
		delete fs_requiredfields["basedn"];
	}
	...
	validSaveForm(function() {
		...
		confirmSave(function() {
			...
			startWaiting();
			jQuery.ajax({
				url: API_URL+"/api/sfte009/insert",
				data: formdata.jsondata,
				headers : formdata.headers,
				type: "POST",
				dataType: "html",
				...
				success: function(data,status,transport){
					...
					successbox(function() { 
						aform.reset();
						setTimeout(function() { $("#domainname").focus(); },200);
					});
...
function update(aform) {
	...
	validSaveForm(function() {
		...
		confirmUpdate(function() {
			...
			startWaiting();
			jQuery.ajax({
				url: API_URL+"/api/sfte009/update",
				data: formdata.jsondata,
				headers : formdata.headers,
				type: "POST",
				dataType: "html",
...
function submitRetrieve(src,domainid) {
	//#(250000) programmer code begin;
	//#(250000) programmer code end;
	var aform = fslistform;
	aform.domainid.value = domainid;
	let formdata = serializeDataForm(aform);
	startWaiting();
	jQuery.ajax({
		url: API_URL+"/api/sfte009/retrieval",
		data: formdata.jsondata,
		headers: formdata.headers,
		type: "POST",
		dataType: "html",
...
function deleteRecord(fsParams) {
	...
	startWaiting();
	jQuery.ajax({
		url: API_URL+"/api/sfte009/remove",
		data: formdata.jsondata,
		headers: formdata.headers,
		type: "POST",
		dataType: "html",
...
function setupInputs() {
	$("#domaintype").change(function() {
		let $input = $("#basedn");
		if("S"==$(this).val()) {
			$input.removeClass("alert-input is-invalid");
			$("#basednrequiredlabel").html("");
			$input.unbind("blur");
			$("#basedn_alert").hide();
			$input.parent().removeClass("has-error");
		} else {			
			$("#basedn").addClass("alert-input");
			$("#basednrequiredlabel").html("*");
			$input.unbind("blur");
